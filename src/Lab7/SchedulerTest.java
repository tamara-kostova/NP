...
class Scheduler <T>{
    ...
    public T next(){
        Date date = timestamps.keySet().stream().filter(d -> d.after(new Date())).collect(Collectors.toList()).get(0);
        return timestamps.get(date);
    }
    public T last(){
        List<Date> dates = timestamps.keySet().stream().filter(d -> d.before(new Date())).collect(Collectors.toList());
        return timestamps.get(dates.get(dates.size()-1));
    }
    public ArrayList<T>getAll(Date begin,Date end){
        return (ArrayList<T>) timestamps.keySet().stream().filter(date -> date.after(begin)&&date.before(end))
                .map(key->timestamps.get(key)).collect(Collectors.toList());
    }
    public T getFirst(){
        return timestamps.values().stream().collect(Collectors.toList()).get(0);
    }
    public T getLast(){
        List<T> dates = timestamps.values().stream().collect(Collectors.toList());
