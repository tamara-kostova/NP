...
class Timestamp<T> implements Comparable<Timestamp<? extends T>>{
    ...
    public String toString() {
        return String.format("%s %s",time,element.toString());
    }
}
@SuppressWarnings("unchecked")
class Scheduler<T>{
    ...
    public Timestamp<T> next(){
        Timestamp now = new Timestamp(LocalDateTime.now(),"now");
        return timestamps.stream().filter(t->t.compareTo(now)>0).min(Comparator.naturalOrder()).orElse(null);
    }
    public Timestamp<T> last(){
        Timestamp now = new Timestamp(LocalDateTime.now(),"now");
        return timestamps.stream().filter(t->t.compareTo(now)<0).max(Comparator.naturalOrder()).orElse(null);
    }
    public List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end){
        return timestamps.stream().filter(timestamp->timestamp.getTime().compareTo(begin)>0&&timestamp.getTime().compareTo(end)<0).collect(Collectors.toList());
    }
}
