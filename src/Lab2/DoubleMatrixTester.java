...
class DoubleMatrix{
    ...
    public double[] toSortedArray(){
        double [] sortedArray = Arrays.stream(matrix).flatMapToDouble(Arrays::stream).sorted().toArray();
        return IntStream.range(0,sortedArray.length).mapToDouble(i->sortedArray[sortedArray.length-i-1]).toArray();
    }

    @Override
    public String toString() {
        return Arrays.stream(matrix).map(row->Arrays.stream(row).mapToObj(x->String.format("%.2f",x)).collect(Collectors.joining("\t"))).collect(Collectors.joining("\n"));
    }
...
class MatrixReader {
    public static DoubleMatrix read(InputStream input) throws InsufficientElementsException{
        ...
        return new DoubleMatrix(matrix,n,m);
    }
}
