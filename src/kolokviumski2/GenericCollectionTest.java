...
class GenericCollection<T extends Comparable<T> & IHasTimestamp> {
    Map<String, Set<T>> byCategories;
    public GenericCollection() {
        this.byCategories = new TreeMap<>();
    }

    public void addGenericItem (String category, T element){
        byCategories.putIfAbsent(category,new TreeSet<>());
        byCategories.get(category).add(element);
    }
    public Collection<T> findAllBetween (LocalDateTime from, LocalDateTime to){
        Collection<T> result = new TreeSet<>(Comparator.reverseOrder());
        byCategories.values().forEach(
                list -> result.addAll(list.stream()
    ...
    public Collection<T> itemsFromCategories (List<String> categories){
        Collection<T> result = new TreeSet<>(Comparator.reverseOrder());
        categories.forEach(category-> result.addAll(byCategories.get(category))
        );
        return result;
    }
        ...
        byCategories.values().forEach(
                list -> list.forEach(
                        element -> {
                            ...
                            byMonthAndDay.putIfAbsent(key,new TreeSet<>(Comparator.reverseOrder()));
                            byMonthAndDay.get(key).add(element);
                        }
                )
        );
    ...
    public Map<Integer, Long> countByYear(){
        ...
        byCategories.values().forEach(
                list -> list.forEach(
                        element -> {
                            byYear.putIfAbsent(element.getTimestamp().getYear(),0l);
                            byYear.replace(element.getTimestamp().getYear(),byYear.get(element.getTimestamp().getYear())+1);
                        }
                )
        );
        return byYear;
